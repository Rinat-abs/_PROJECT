document.addEventListener('DOMContentLoaded', () => {
    let 
    editBtns = document.querySelectorAll('.table__revenue>.table__price__img'),
    editInputs = document.querySelectorAll('.table__revenue>input[name="revenue"]'),
    editTextPrice = document.querySelectorAll('.table__revenue>.price'),
    saveBtns = document.querySelectorAll('.table__revenue>.save_price'),
    priceBeforeEdit = 0
...
saveBtns.forEach((btn, i)=> {
    btn.addEventListener('click', (event) => {
        ...
        btn.setAttribute("data-revenue",editInputs[i].value)
        btn.classList.add('hidden')


        const 
            id = event.target.dataset.id,
            revenue = event.target.dataset.revenue;

      
            fetch('/plan/edit_pharmacy_category/revenue/' + id + '/' + revenue, {
                method: 'post'
            })
            .then(res => {
